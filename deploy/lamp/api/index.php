...
try {
    ...
    // PUT /player/:id/state
    elseif ($method === 'PUT' && preg_match('#^/player/([^/]+)/state$#', $uri, $m)) {
        require __DIR__ . '/routes/player.php';
        savePlayerState($m[1]);
    }
    // PUT /player/:id/conversation/:characterId
    elseif ($method === 'PUT' && preg_match('#^/player/([^/]+)/conversation/([^/]+)$#', $uri, $m)) {
        require __DIR__ . '/routes/player.php';
    ...
    // PUT /player/:id/board
    elseif ($method === 'PUT' && preg_match('#^/player/([^/]+)/board$#', $uri, $m)) {
        require __DIR__ . '/routes/player.php';
        saveBoard($m[1]);
    }
    ...
    elseif ($method === 'PUT' && preg_match('#^/player/([^/]+)/notebook$#', $uri, $m)) {
        require __DIR__ . '/routes/player.php';
        saveNotebook($m[1]);
    }
    else {
